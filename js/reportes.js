    ...
    setTimeout(() => {
        try {
            ...
            const resumenHeaders = [
                ...
                'Capacitación (h)', 'Admin. (h)', 'C. Virtuales (h)', 'Total (h)', 'Horas Extras (h)', '% Ocupación'
            ];
            const resumenData = [
            ...
            (estado.profesores || []).sort((a, b) => (a.nombre||'').localeCompare(b.nombre||'')).forEach(prof => {
                ...
                resumenData.push([
                    ...
                    carga.cursosVirtuales || 0, carga.total || 0, carga.horasExtras || 0,
...
function generarReporteGlobalPDFCompleto(tamanio, orientacion) {
    ...
    try {
        ...
        const encabezados = ['Nombre', 'Contrato', 'Clases (h)', 'Total (h)', '% Ocup.', 'Extras (h)'];
        const anchosBase = [0.30, 0.18, 0.12, 0.12, 0.13, 0.15];
        ...
        (estado.profesores || []).sort((a,b) => (a.nombre||'').localeCompare(b.nombre||'')).forEach(prof => {
            ...
            const datosFila = [
                prof.nombre || 'N/A', obtenerTextoTipoContrato(carga.tipoContrato), carga.clases || 0, carga.total || 0,
                (carga.porcentajeOcupacion || 0) + '%', carga.horasExtras || 0
            ];
            datosFila.forEach((dato, idx) => {
                let xPos = margen + (anchosColumna.slice(0, idx).reduce((sum, w) => sum + w, 0));
                if (idx === 5 && parseFloat(carga.horasExtras) > 0) doc.setTextColor(255, 0, 0);
                doc.text(String(dato), xPos + 2, y, {maxWidth: anchosColumna[idx] - 4});
                if (idx === 5 && parseFloat(carga.horasExtras) > 0) doc.setTextColor(0, 0, 0);
            });
            y += 7;
            contadorProfesores++;
        });
